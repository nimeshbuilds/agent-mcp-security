...
A later portability follow-up adds four regression tests (**22 harness tests total**). The supplied source root is checked for a symlink before canonicalization; confined relative reads still reject child links, traversal, oversized files and changed digests. This fixes Windows short-path and relative-root aliases. The exact same 153 cases and 20 payloads were prepared again with zero evidence errors and no model calls. Repository LF attributes also preserve historical raw-byte digests on Windows. [Executed follow-up evidence](../validation-v010/portability-receipt.json).
