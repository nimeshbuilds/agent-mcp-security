...
The quickstart verifies `invscan`, `invarune` and `ai-security-scan`, detailed help, offline inventory/explanations, exact selections, source/image/skill scans, terminal-only behavior, review exceptions, real PDF field edits and fresh rescans. Authentication and live model behavior require separate evidence.

Implementation commit `2a79751a4d49152dfc0a1031d9a06f84ac9bf7d7` passed [all eight CI jobs](https://github.com/nimeshbuilds/invarune/actions/runs/35946791805): Linux Python 3.9/3.12/3.14, macOS Python 3.12, Windows Python 3.12, Docker built-image integration, Headroom integration and package/schema validation. Each unit suite counted 1,016 tests: Linux base jobs skipped 36 optional-PDF or case-alias cases, macOS skipped 32 optional-PDF cases, Windows skipped 40 optional-PDF/platform/filesystem cases, and the QA package job skipped four case-alias cases that require a case-insensitive filesystem. All executed tests passed. The installed Windows quickstart passed **48 steps** with PDF/gateway checks excluded; the Linux Headroom quickstart passed **56 steps** with PDF excluded. The [CI receipt](ci-receipt.json) retains every job and exact skip reasons.

The same commit's [GitHub Pages build and deployment](https://github.com/nimeshbuilds/invarune/actions/runs/35946791758) both succeeded. The [release/download verification](release-receipt.json) separately records downloaded-asset hashes, fresh installed-wheel checks and public-site verification; a successful local build alone is not used as evidence for those outcomes.
