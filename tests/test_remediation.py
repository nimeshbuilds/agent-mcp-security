...
class RemediationTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        corpus['cases'].extend(json.loads((ROOT / 'benchmarks' / 'callflow_permissions_accuracy.json').read_text(encoding='utf-8'))['cases'])
        cls.positives = {}
    ...
    def test_every_detector_positive_receives_its_own_complete_plan(self):
        ...
        self.assertEqual(len(self.positives), 47)
        self.assertEqual({entry['rule_id'] for entry in self.catalog['rules']}, set(RULE_BY_ID))
        self.assertEqual(len(self.catalog['rules']), 47)
        self.assertEqual(set(self.guidance), {finding['id'] for finding in self.report['findings']})
        self.assertEqual(len({entry['agent_mcp_relevance'] for entry in self.guidance.values()}), 47)
        self.assertEqual(len({entry['summary'] for entry in self.guidance.values()}), 47)
    ...
    def test_replacements_cover_distinct_implementation_and_runtime_cases(self):
        ...
        required = {
            ...
            'AI047': ['0o600', '0o700', 'acl', 'ownership', 'tenant'],
        }
