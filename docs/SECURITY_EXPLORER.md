...
Install the current checkout as described in the [quickstart](QUICKSTART.md). The previously published 0.11.0 wheel predates these explorer commands. The `invarune` and `ai-security-scan` aliases support the same flags when installed from 0.12.0.
