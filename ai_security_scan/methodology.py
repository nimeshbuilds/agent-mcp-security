...
AREAS = (
    ("Execution and data flow",
     "Python AST checks with bounded local aliases/values; JavaScript/TypeScript tokens and balanced calls identify selected eval, shell, deserialization, SQL and template patterns.",
     "A model can explain a retrieved call path, question whether input is trusted and identify contextual concerns the pattern rules do not encode.",
     "Cross-file flows, dynamic imports, wrappers, reflection, generated code, unsupported languages and runtime values can hide a risk. A dangerous API can also be intentional and correctly constrained.",
...
def build_methodology(report):
    ...
    return {
        ...
        "workflow": [
            ...
            "Run advisory model requests under fixed limits; validate response schemas, known identifiers and exact evidence quotes. The model cannot select files or execute tools through the review controller.",
